...
const MediaPlayerContext = React.createContext(null);
...
function MediaPlayerImpl(props) {
  ...
  const volumeIndicatorTimeoutRef = React.useRef(null);
...
function MediaPlayerError(props) {
  ...
  return (
    <ErrorPrimitive
      ...
      {children ?? (
        <div
          className="flex max-w-md flex-col items-center gap-4 px-6 py-8 text-center">
...
function MediaPlayerVolumeIndicator(props) {
  ...
  return (
    <VolumeIndicatorPrimitive
      ...
      <div
        className="fade-in-0 zoom-in-95 flex animate-in flex-col items-center gap-3 rounded-lg bg-black/30 px-6 py-4 text-white backdrop-blur-xs duration-200">
        <div className="flex items-center gap-2">
...
function MediaPlayerSeekBackward(props) {
  ...
  const onSeekBackward = React.useCallback((event) => {
    props.onClick?.(event);
...
function MediaPlayerSeekForward(props) {
  ...
  const onSeekForward = React.useCallback((event) => {
    props.onClick?.(event);
...
function MediaPlayerSeek(props) {
  ...
  const collisionDataRef = React.useRef(null);
  ...
  const timeCache = React.useRef(new Map());
  ...
  const getCachedTime = React.useCallback((time, duration) => {
  ...
  const onCollisionDataUpdate = React.useCallback(() => {
    ...
    const boundaries = tooltipCollisionBoundary
      ? Array.isArray(tooltipCollisionBoundary)
        ? tooltipCollisionBoundary
        : [tooltipCollisionBoundary]
      : ([context.rootRef.current].filter(Boolean));
  ...
  const getCurrentChapterCue = React.useCallback((time) => {
  ...
  const getThumbnail = React.useCallback((time) => {
  ...
  const onPreviewUpdate = React.useCallback((time) => {
  ...
  const onTooltipPositionUpdate = React.useCallback((clientX) => {
  ...
  const onSeekCommit = React.useCallback((value) => {
    const time = value[0] ?? 0;
...
function MediaPlayerVolume(props) {
  ...
  const onVolumeCommit = React.useCallback((value) => {
    const volume = value[0] ?? 0;
  ...
  return (
    <div
      ...
      className={cn("group flex items-center", expandable
      ...
      <SliderPrimitive.Root
        ...
        max={1}
        step={0.1}
        className={cn("relative flex touch-none select-none items-center", expandable
...
function MediaPlayerPlaybackSpeed(props) {
  ...
  const onPlaybackRateChange = React.useCallback((rate) => {
    dispatch({
      type: MediaActionTypes.MEDIA_PLAYBACK_RATE_REQUEST,
      detail: rate,
    });
...
function MediaPlayerLoop(props) {
  ...
  const onLoopToggle = React.useCallback((event) => {
    props.onClick?.(event);
    if (event.defaultPrevented) return;
...
function MediaPlayerFullscreen(props) {
  ...
  const onFullscreen = React.useCallback((event) => {
    props.onClick?.(event);
...
function MediaPlayerPiP(props) {
  ...
  const onPictureInPicture = React.useCallback((event) => {
    props.onClick?.(event);
...
function MediaPlayerCaptions(props) {
  ...
  const isDisabled = disabled || context.disabled;
  const onCaptionsToggle = React.useCallback((event) => {
    props.onClick?.(event);
...
function MediaPlayerSettings(props) {
  ...
  const onRenditionChange = React.useCallback((renditionId) => {
    dispatch({
  ...
  const onShowSubtitleTrack = React.useCallback((subtitleTrack) => {
    dispatch({
      type: MediaActionTypes.MEDIA_TOGGLE_SUBTITLES_REQUEST,
      detail: false,
    });
        ...
        <DropdownMenuSub>
          ...
          <DropdownMenuSubContent>
            ...
            {mediaSubtitlesList.map((subtitleTrack) => {
              const isSelected = mediaSubtitlesShowing.some((showingSubtitle) =>
...
export { MediaPlayer, MediaPlayerVideo, MediaPlayerAudio, MediaPlayerControls, MediaPlayerControlsOverlay, MediaPlayerLoading, MediaPlayerError, MediaPlayerVolumeIndicator, MediaPlayerPlay, MediaPlayerSeekBackward, MediaPlayerSeekForward, MediaPlayerSeek, MediaPlayerVolume, MediaPlayerTime, MediaPlayerPlaybackSpeed, MediaPlayerLoop, MediaPlayerFullscreen, MediaPlayerPiP, MediaPlayerCaptions, MediaPlayerDownload, MediaPlayerSettings, MediaPlayerPortal, MediaPlayerTooltip, //
useMediaSelector as useMediaPlayer, useStore as useMediaPlayerStore };
