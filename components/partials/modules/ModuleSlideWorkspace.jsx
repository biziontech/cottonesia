...
import { PanelLeft, Image as ImageIcon, Layers } from "lucide-react";
...
export const ModuleSlideWorkspace = () => {
    ...
    const handleSlideUpdate = async (updates, content = null, slideType = null, thumbnail = null) => {
        ...
        if (content && slideType) {
            try {
                ...
                // Upload ke server
                const response = await fetch(`${API_URL}/office/training-slides/${module?.uuid}/update/${selectedSlide?.ulid}`, {
                    method: 'POST',
                    headers: {
    ...
    const handleAttachmentDelete = async (payload) => {
        setSelectedSlide(prev => ({
            ...prev,
            ...payload
        }));
        ...
        try {
            const response = await api.fetch(`${API_URL}/office/training-slides/${module?.uuid}/delete-attachment/${selectedSlide?.ulid}`, {
                method: 'DELETE'
            });
            ...
            if (!response?.success) {
                throw new Error(data.message || 'Gagal menghapus attactment');
            }
            ...
            toast.success(data.message || "Attactment berhasil dihapus")

        } catch (error) {
            toast.error(error.message || "Terjadi kesalahan saat menghapus attactment");
        }
    };

    const handleAttachmentSave = async (slide) => {
        try {
            const formData = new FormData();

            // Handle berdasarkan attachment type
            if (slide?.attachment_type === 'audio') {
                if (slide?.attachment_audio_file) {
                    formData.append('attachment_audio_file', slide.attachment_audio_file, `record-${Date.now()}.wav`);
                } else {
                    toast.error('File audio tidak valid');
                    return;
                }
            } else if (slide?.attachment_type === 'text') {
                if (slide?.attachment_content) {
                    formData.append('attachment_content', slide.attachment_content);
                } else {
                    toast.error('Konten teks tidak boleh kosong');
                    return;
                }
            } else {
                toast.error('Tipe attachment tidak valid');
                return;
            }

            // Upload ke server
            const response = await fetch(`${API_URL}/office/training-slides/${module?.uuid}/update-attachment/${slide?.ulid}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${api.token}`,
                },
    ...
    return (
        <div className='flex flex-1 flex-col gap-5 items-center justify-start relative px-3 py-5'>
            <div className="flex flex-col gap-4 w-full h-[calc(100dvh-209px)] overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                <div className='max-w-5xl w-full mx-auto relative flex flex-col gap-3'>
