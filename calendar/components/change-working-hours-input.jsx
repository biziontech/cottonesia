    ...
    return (<div className="flex flex-col gap-2">
      ...
      <div className="space-y-4">
        {DAYS_OF_WEEK.map(day => {
            ...
            return (<div key={day.index} className="flex items-center gap-4">
              <div className="flex w-40 items-center gap-2">
                <Switch checked={isDayActive} onCheckedChange={() => handleToggleDay(day.index)}/>
                <span className="text-sm font-medium">{day.name}</span>
              </div>

              {isDayActive ? (<div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <span>From</span>
                    <TimeInput id={`${day.name.toLowerCase()}-from`} hourCycle={12} granularity="hour" value={{ hour: localWorkingHours[day.index].from, minute: 0 }} onChange={value => handleTimeChange(day.index, "from", value)}/>
                  </div>

                  <div className="flex items-center gap-2">
                    <span>To</span>
                    <TimeInput id={`${day.name.toLowerCase()}-to`} hourCycle={12} granularity="hour" value={{ hour: localWorkingHours[day.index].to, minute: 0 }} onChange={value => handleTimeChange(day.index, "to", value)}/>
                  </div>
